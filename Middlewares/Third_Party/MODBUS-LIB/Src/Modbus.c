...
uint8_t test_step1 = 0;
uint8_t test_step2 = 0;
uint8_t test_step3 = 0;


uint8_t JogX_Busy_Flag = 0;
uint8_t JogY_Busy_Flag = 0;
uint8_t JogZ_Busy_Flag = 0;

int32_t JogX_Axis_Count = 0;
int32_t JogY_Axis_Count = 0;
int32_t JogZ_Axis_Count = 0;
...
//	mHandlers[0]->u8Buffer[3] = HAL_GPIO_ReadPin(GPIOF, GPIO_PIN_12); // DUC ADD THEM
    sendTxBuffer(modH);
...
/*			uint16_t sfd = 59;
	//    	modH->u8Buffer[ modH->u8BufferSize ] = highByte(modH->u16regs[i]);
	    	modH->u8Buffer[ modH->u8BufferSize ] = sfd/256;
//			modH->u8Buffer[ modH->u8BufferSize ] = ((milisec>> 24)&0x000000FF);
			modH->u8BufferSize++;
	//    	modH->u8Buffer[ modH->u8BufferSize ] = lowByte(modH->u16regs[i]);
			modH->u8Buffer[ modH->u8BufferSize ] = sfd%256;
			modH->u8BufferSize++;
		}*/
    	uint32_t sfd = 655322225;
/*    	modH->u8Buffer[ 3 ] = ((sfd>> 8)&0x000000FF);
    	modH->u8Buffer[ 4 ] = ((sfd>> 0)&0x000000FF);*/
       	modH->u8Buffer[ 3 ] = (uint8_t)((sfd>> 24)&0x000000FF);
       	modH->u8Buffer[ 4 ] = (uint8_t)((sfd>> 16)&0x000000FF);
       	modH->u8Buffer[ 5 ] = (uint8_t)((sfd>> 8)&0x000000FF);
       	modH->u8Buffer[ 6 ] = (uint8_t)((sfd>> 0)&0x000000FF);
        modH->u8BufferSize         = 7;
    }
//    }
/*    if(u16StartAdd == 40)
    {
		for (i = u16StartAdd; i < u16StartAdd + u8regsno; i++)
		{
			uint16_t coz = 65535;
	//    	modH->u8Buffer[ modH->u8BufferSize ] = highByte(modH->u16regs[i]);
	//    	modH->u8Buffer[ modH->u8BufferSize ] = milisec;
			modH->u8Buffer[ modH->u8BufferSize ] = coz/256;
			modH->u8BufferSize++;
	//    	modH->u8Buffer[ modH->u8BufferSize ] = lowByte(modH->u16regs[i]);
			modH->u8Buffer[ modH->u8BufferSize ] = coz%256;
			modH->u8BufferSize++;
...
int8_t process_FC5( modbusHandler_t *modH )
{
    ...
    switch(u16coil)
    {
		case 3:
		{
		    if(mHandlers[0]->xBufferRX.uxBuffer[4] == 0xFF)
		    {
		        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_3, GPIO_PIN_RESET);
		    }
		    else if (mHandlers[0]->xBufferRX.uxBuffer[4] == 0x00)
		    {
		...
		case 16:
		{
		    if(mHandlers[0]->xBufferRX.uxBuffer[4] == 0xFF)
		    {
				  ...
				  {
					  Move_XYZ_ABC(20000, 20000,20000,0,0,0,1990,5);
					  JogX_Axis_Count = Current_PosX;
					  JogY_Axis_Count = Current_PosY;
					  JogZ_Axis_Count = Current_PosZ;
				  }
		    }
		    else if (mHandlers[0]->xBufferRX.uxBuffer[4] == 0x00)
		    {
				  if(Busy_Flag_XYZ == 0 && Origin_Flag == 0 )
				  {
					  Move_XYZ_ABC(1000, 1000,1000,0,0,0,1990,5);
					  JogX_Axis_Count = Current_PosX;
					  JogY_Axis_Count = Current_PosY;
					  JogZ_Axis_Count = Current_PosZ;
				  }
